'''
Script to plot the figure S.9 - performance of the double mutant targeting DDPA and GLUDy only
'''
...
for conc, scaling in concentrations_to_plot.items():
    ...
    plt.fill_between(time, df_designs_lower[conc] * scaling, df_designs_upper[conc] * scaling,
                     ...
                     interpolate=True,
                     alpha=0.1)
    df_temp = pd.concat([df_wt_mean[conc] * scaling,
                         df_wt_lower[conc] * scaling,
                         df_wt_upper[conc] * scaling,
                         df_designs_mean[conc] * scaling,
                         df_designs_lower[conc] * scaling,
                         df_designs_upper[conc] * scaling
                         ],
                        axis=1)
    df_temp.columns = [conc + '_' + i for i in ['wt_median', 'wt_lower', 'wt_upper', 'designs_median', 'designs_lower', 'designs_upper']]
    source_data.append(df_temp)

    plt.legend()
    plt.xlabel('Time (h)')
    plt.ylabel(ylabels[conc])
    plt.savefig(folder_for_output + '/figure_S.9_{}.png'.format(conc))
    plt.close()
