"""
Script to generate the plot for Supplementary Figure S.6 (Supplementary note V)
...
Changed to add the source data extraction code
"""
...
kinetic_models = ['1712,6', '1715,6', '1717,6', '2392,6', '2482,8', '3927,1', '4230,7', '4241,5', '4456,7',
                   '4468,6']
folder_for_output = '../../output/figures/figure-S.6/'
...
for this_model in kinetic_models:
    ...
    data_folder = './../../output/data/S.V-aroGtktA-analysis/reactor-verification/{}'.format(this_model)
...
for conc, scaling in concentrations_to_plot.items():
    ...
    df_temp = pd.concat([dfs_wt_median[conc] * scaling,
                         dfs_ddpa_tkt_shiki_median[conc] * scaling,
                         dfs_ddpa_tkt_shiki_lower[conc] * scaling,
                         dfs_ddpa_tkt_shiki_upper[conc] * scaling,
                         dfs_ddpa_tkt_shiki_median[conc] * scaling
                         ],
                        axis=1)
    df_temp.columns = [conc + i for i in ['_wt_median','_aroG_tktA_new_median','_aroG_tktA_new_lower','_aroG_tktA_new_upper','_aroG_tkt_old_median']]
    ...
    plt.savefig(folder_for_output + '/Figure_S.6_{}.png'.format(conc))
    plt.close()
