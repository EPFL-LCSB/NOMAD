"""
In this script, we identify the top 5 designs by mean NRA predicted increase in anthranilate yield
and verify them in a reactor setup using their model specific NRA suggested fold changes in enzyme activity
...
folder_to_designs = './../../output/data/eK_trpD9923'
path_to_designs = folder_to_designs + '/all_unique_designs_w_enz_fold_changes.csv'
folder_for_output = './../../output/verification-top-5-designs-reactor/eK_trpD9923'
...
for this_model in kinetic_models:
    ...
    """    
    Test each top design in a reactor setting for this kinetic model    
    """
    ...
    for ix_design, this_design in this_model_designs.groupby('design_ix'):
        ...
        # All designs already have DDPA --> remove regulation also
        # Remove the regulation of DDPA by phenylalanine
        if 'EU_DDPA' in list(this_design['enzyme']):
            reactor.parameters['strain_1_ki_inhibitor1_DDPA'].value = 1e42
            reactor.parameters['strain_1_k_inhibition_IM_phe_L_c_DDPA'].value = 1e42
        ...
        dict_ = {'model': this_model,
                 'design_ix': ix_design,
                 'E1': this_design['enzyme'].iloc[0],
                 'E2': this_design['enzyme'].iloc[1],
                 'E3': this_design['enzyme'].iloc[2],
