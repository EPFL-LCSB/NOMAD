"""
This script generates designs for overproducing anthranilate in eK_trpD9923_d2
"""
import numpy as np
import pandas as pd
...
calc_fluxes = make_flux_fun(reactor.models.strain_1, QSSA)

kinetic_models = list(parameter_population._index.keys())

def reset_reactor(concs_, params_):

    # Parameterize the rector and initialize with incolum and medium data
    reactor.parametrize(params_, 'strain_1')
    reactor.initialize(concs_, 'strain_1')
...
for this_model in kinetic_models:
    ...
    output_folder = './../../output/data/eK_trpD9923_d2/{}/'.format(this_model)
    ...
    """
    2. SIMULATE DOUBLE MUTANT AND GET NEW STEADY STATE
    """
    reset_reactor(concs_=concentrations, params_=parameter_set)

    # Remove regulation of DDPA by Phe and upregulate tktA
    ...
    sol_d2 = reactor.solve_ode(np.linspace(0, TOTAL_TIME, N_STEPS),
                                solver_type='cvode',
                                rtol=1e-9,
                                atol=1e-9,
                                max_steps=1e9,
    ...
    ix_st_st = 150 # time point at which we get steady-state concentrations -- 9 hours
    new_concentrations = sol_d2.concentrations.iloc[ix_st_st]
    params = {k: v.value for k, v in reactor.parameters.items()}
    new_fluxes = pd.Series(calc_fluxes(new_concentrations, parameters=params))
    new_concentrations.index = [i.replace('strain_1_', '') for i in new_concentrations.index]
    
    # Change the parameter set values as well - IMPORTANT - this way we don't change the original parameter set and parameter population
    ...
    """
    NRA analysis at the steady state of d2
    """
    # Get flux + concentrationc control coefficients and calculate yield control coefficients for anthranilate wrt glc
    fcc = kmodel.flux_control_fun(new_fluxes, new_concentrations, [new_parameter_set]).mean('sample')
    ccc = kmodel.concentration_control_fun(new_fluxes, new_concentrations, [new_parameter_set]).mean('sample')
    ...
    if SIMULATE_IN_REACTOR:
        ...
        ''' 1. Simulation of the wildtype strain'''
        reset_reactor(concs_=concentrations, params_=parameter_set)
        start = T.time()
        ...
        ''' 5. Strain designs using d2'''
        sol_ode_alt = []
        for this_design in designs:
            # Reset reactor but remember to use new steady state as the starting point!!
            reset_reactor(concs_=new_concentrations, params_=parameter_set)
            ...
            # Target existing interventions in d2 (aroG fbr and tktA)
            reactor.parameters['strain_1_ki_inhibitor1_DDPA'].value = 1e42
            reactor.parameters['strain_1_k_inhibition_IM_phe_L_c_DDPA'].value = 1e42
            reactor.parameters['strain_1_vmax_forward_TKT1'].value *= 10
            reactor.parameters['strain_1_vmax_forward_TKT2'].value *= 10
    ...
    # Save steady state fluxes and concentrations for d2 to file
    new_fluxes.to_csv(output_folder + 'steady_state_fluxes.csv')
    new_concentrations.to_csv(output_folder + 'steady_state_concentrations.csv')
