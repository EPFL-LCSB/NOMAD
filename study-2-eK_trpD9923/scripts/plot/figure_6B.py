...
LIST_OF_MODELS = ['1618,484', '3717,576', '3970,358', '4011,287', '4012,339', '4015,392', '4024,450',
 '4026,483', '4027,482', '4028,498', '4029,474', '4057,456', '4693,457']
styles = ['ko', 'kx', 'k^']
folder_to_exp_data = './../../data/fermentation-experiments/'
folder_to_ode_sols = './../../output/data/eK_trpD9923/'
...
dfs_wt, dfs_d1, dfs_d2 = [], [], []
for ix in LIST_OF_MODELS:
    filename = folder_to_ode_sols + '{}/ode_solutions.csv'.format(ix)
    df = pd.read_csv(filename, index_col = 0)
    dfs_wt.append(df[df['solution_id'] == 0])  # Only wt
    dfs_d1.append(df[df['solution_id'] == 2])
    dfs_d2.append(df[df['solution_id'] == 3])
dfs_wt = pd.concat(dfs_wt)
dfs_d1 = pd.concat(dfs_d1)
dfs_d2 = pd.concat(dfs_d2)
