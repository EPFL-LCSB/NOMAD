"""
Phenotype Perturbation Sensitivity analysis - Supplementary note VI

For each combination of max fold enz activity and max fold conc change do the following
- For each model
-- enumerate top designs within 99%
-- implement designs in reactor

Results are used to plot figure S.9 (./plot/figure_S.9.py)
"""
import numpy as np
import pandas as pd
...
'''
for this_fold_conc_change in MAX_FOLD_CONC_CHANGES:
    for this_fold_enz_activity in MAX_FOLD_ENZ_ACTIVITIES:
        for this_model in kinetic_models:
            ...
            nmodel.max_fold_enzyme_change = np.log(this_fold_enz_activity)
            nmodel.max_fold_concentration_change = np.log(this_fold_conc_change)
            ...
            # Enumerate just the top alternative
            _, designs, _ = enumerate_solutions(nmodel, max_iter=1, threshold=0.99)
