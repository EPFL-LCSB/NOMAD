"""
In this script, we analyse why the insilico aroGfbr + TKT titer is so low (Supplementary note VIII)
Since we don't know what other modifications are taking place within the cell we use NRA to tell us the following
- If I enforce DDPA + TKT1/2 upregulation, and allow 4 more enzyme modifications, how far can I reach?
- Here we generate NRA designs for each model allowing a 5 fold concentration change

The data generated by this script is then collated and the top most frequently occuring design is implemented in
bioreactor simulations. These are done by the next two scripts.
"""
...
MAX_N_ENZ = 7
SIMULATE_IN_REACTOR = False # No simulations here - we generate designs and only simulate the more frequently occuring one
...
for this_model in kinetic_models:
    ...
    # Ensure DDPA, TKT1, TKT2 are all upregulated
    nmodel.variables.EU_TKT1.lb = 0.01
    nmodel.variables.EU_TKT2.lb = 0.01
    nmodel.variables.EU_DDPA.lb = 0.01
    ...
    # Enumerate alternatives - max 20 iterations or upto 99% of the solution
    _, designs, _ = enumerate_solutions(nmodel, max_iter=20, threshold=0.99)
